...
func init() {
	Register("bash_scan", &BashScanConsumer{})

}

type BashScanConsumer struct {
}

func (c *BashScanConsumer) Handle(messageValue []byte, producer sarama.AsyncProducer, topic string) {
	...
	for _, msg := range processInfoMsg.CommandList {
		...
		bodyMap["UserName"] = string(processInfoMsg.UserName)
		bodyMap["Platform"] = processInfoMsg.Platform

		allMap := map[string]interface{}{"Head": headMap, "UserCommandReport": bodyMap}
