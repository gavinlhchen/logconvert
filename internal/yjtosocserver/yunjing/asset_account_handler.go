...
func init() {
	Register("asset_account", &AssetAccountConsumer{})

}

type AssetAccountConsumer struct {
}

func (c *AssetAccountConsumer) Handle(messageValue []byte, producer sarama.AsyncProducer, topic string) {
	...
	for _, msg := range processInfoMsg.AccountList {
		...
		allMap := map[string]interface{}{"Head": headMap, "UserAccountInfo": bodyMap}
